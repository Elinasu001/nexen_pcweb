...
//차량 번호 검색 버튼 클릭 이벤트 > 나이스 DNR 연동 ver1
/*function niceCarInfo(data) {
	//화면 전환
	screen_change( 'num' , '2' );

	//차 정보 + 연식
	$(".sizeFinder .size-tab.type-num.step-02 .img-wrap").html('<img src="' + data.carInfo.CAR_CLASS_REP_IMAGE + '">');
	var viewFuelNm = '';
	...
	$(".sizeFinder .size-tab.type-num.step-02 .car-name").text(data.carInfo.BRAND_NM + " " + data.carInfo.CAR_CLASS_NM + " " + viewFuelNm);
	$(".sizeFinder .size-tab.type-num.step-02 .car-year > span").text(data.carInfo.YEAR_TYPE+"년");
	// $(".sizeFinder .size-tab.type-num.step-02 .size").text('235/55R19');

	//등급
	var niceCarGradeStr = "";
	for(var i=0; i<data.gradeList.length; i++) {
		if(data.gradeList[i].RECOMM_GRADE_YN == 'Y') {
			niceCarGradeStr = data.gradeList[i].CAR_GRADE_NM;
		}
	}

	var param = {};

	param["repCarClassNbr"] = data.carInfo.REP_CAR_CLASS_NBR;
	param["carClassNm"] = data.carInfo.CAR_CLASS_NM;
	param["fuel"] = data.carInfo.FUEL;
	param["brdDivCd"] = viewFuelNm;//안쓸것 같은 이름을 단순히 찾은것이다. 실제로는 연료명이다.
	param["yearType"] = data.carInfo.YEAR_TYPE;
	param["brandNbr"] = data.carInfo.BRAND_NM;
...
//차량 번호 검색 버튼 클릭 이벤트 > 나이스 DNR 연동 ver2
function niceCarInfo(data) {
	//화면 전환
	screen_change( 'num' , '2' );

	//차 정보 + 연식 CLASS_MODEL_IMG
	//$(".sizeFinder .size-tab.type-num.step-02 .img-wrap").html('<img src="' + data.carInfo.CAR_CLASS_REP_IMAGE + '">'); //AS_IS
	$(".sizeFinder .size-tab.type-num.step-02 .img-wrap").html('<img src="' + data.carInfo.classModelImg + '">'); //TO_BE
	var viewFuelNm = '';
	$.each(feulCmmCd, function( i, cmmItem ){
		if(cmmItem.cmmCd == data.carInfo.fuel) {
			viewFuelNm = cmmItem.cmmCdNm;
	...
	//$(".sizeFinder .size-tab.type-num.step-02 .car-name").text(data.carInfo.BRAND_NM + " " + data.carInfo.CAR_CLASS_NM + " " + viewFuelNm);  //AS_IS
	$(".sizeFinder .size-tab.type-num.step-02 .car-name").text(data.carInfo.makerNm + " " + data.carInfo.modelNm + " " + viewFuelNm); //TO_BE
	...
	for(var i=0; i<data.gradeList.length; i++) {
		if(data.gradeList[i].recommGradeYn == 'Y') {
			//niceCarGradeStr = data.gradeList[i].CAR_GRADE_NM; //AS_IS
			niceCarGradeStr = data.gradeList[i].gradeNm;//TO_BE
		}
	}

	var param = {};

	//param["repCarClassNbr"] = data.carInfo.REP_CAR_CLASS_NBR;//AS_IS
	param["repCarClassNbr"] = data.carInfo.classModelId; //TO_BE
	//param["carClassNm"] = data.carInfo.CAR_CLASS_NM;//AS_IS
	param["carClassNm"] = data.carInfo.modelNm;
	param["fuel"] = data.carInfo.fuel;
	param["brdDivCd"] = viewFuelNm;//안쓸것 같은 이름을 단순히 찾은것이다. 실제로는 연료명이다.
	param["yearType"] = data.carInfo.prye;
	//param["brandNbr"] = data.carInfo.BRAND_NM; //AS_IS
	param["brandNbr"] = data.carInfo.makerNm; //TO_BE
...
function niceGradeList(pData, niceCarGrade, pParam) {
	...
	//사이즈
	$.ajax({
	    url: "/product/selectSizeFinderListRental",
	    type: "POST",
	    cache:false,
	    contentType:"application/json;charset=UTF-8",
...
function niceSizeList(pData) {
	var carTireSizeHtml = '';
	for(var i=0; i<pData.length; i++) {
		if (pData[i] != null){
			carTireSizeHtml += '<li><a data-code="'+ pData[i].classCd +'|' + pData[i].driveFwd + '|' +pData[i].driveRwd+ '">' + pData[i].tireSpec + '</a></li>';
		}
	}
...
function selectChoiceGradeSizeList() {
	...
	//등급+사이즈
	$.ajax({
	    url: "/product/selectSizeFinderSizeRental",
	    type: "POST",
	    cache:false,
	    contentType:"application/json;charset=UTF-8",
	    data: JSON.stringify(param),
...
$(function(){
	...
	$.ajax({
		    ...
		    type: "POST",
		    cache:false,
		    data: {carNo:niceCarNo, usrNm:niceUsrNm},
		    success: function(data) {
		    	//$("#loadingWrap").hide();
		    	//console.log(":::niceDnr:::");
		    	//console.log(data);
		    	if(data.code == "000") {
			    	...
			    	$("#loadingWrap").hide();
			    	ComUtil.alert("[" + data.code + "]" + "시스템 트래픽이 많아 차량 정보 조회에 실패하였습니다. 다시 한번 시도해 보시고 계속 발생 시 차종찾기 기능을 이용 바랍니다.");
			    }
		    },
		    error: function (request, status, error) {
		    	$("#loadingWrap").hide();
	...
	$("#findSizeNumCheck").on('click',function(){
		...
		console.log(param);
		//나이스 DNR 통신
		console.log("여기로 오는 지확인 주소 변경 -> /product/niceDnrVer2Api");
		$.ajax({
		    url: "/product/niceDnrVer2Api",
		    type: "POST",
		    cache:false,
		    async: true,
		    data: JSON.stringify(param),//{carNo:niceCarNo, usrNm:niceUsrNm},
		    dataType: "JSON",
		    contentType: "application/json; charset=utf-8",
		    success: function(data) {
		    	//$("#loadingWrap").hide();
...
var car_search = {
	...
	step_api:[
		...
		'/product/selectSizeFinderSizeRental',  // 사이즈
	],
	choice_items:[],
	nowStep:0,
	init:function(){
